...
//now to store data in medication history
    ...
    for($i = 0; $i < $size; $i++) {
      ...
      $curDosage = $dosages[$i];
      $breakfast = isset($_POST['breakfast'][$i]) ? 1 : 0; // 1 if checked, 0 if not checked
      $lunch = isset($_POST['lunch'][$i]) ? 1 : 0;
      $dinner = isset($_POST['dinner'][$i]) ? 1 : 0;

      $qeuryMedicationHistory = "INSERT INTO `patient_medication_history`(
      `patient_visit_id`,
      `medicine_details_id`, `quantity`, `dosage`, `breakfast`, `lunch`, `dinner`)
      VALUES($lastInsertId, $curMedicineDetailId, $curQuantity, $curDosage, $breakfast, $lunch, $dinner);";
...
<head>
 ...
 <title>New Prescription - Health Wizards Patient Management System</title>
...
include './config/sidebar.php';?>  
  ...
  <div class="row">
    ...
    <div class="col-md-2">
      <button type="button" id="viewPdfButton" class="btn btn-primary btn-sm btn-flat btn-block">View Prescription PDF</button>
    </div>
  </div>
</form>
...
<script>
  ...
  $(document).ready(function() {
    ...
    $('#viewPdfButton').click(function() {
      ...
      var pdfUrl = 'prescription.pdf'; // Replace with the actual PDF file path
    ...
    $("#add_to_list").click(function() {
      ...
      var breakfastChecked = $("input[name='breakfast']:checked").length > 0 ? "yes" : "no";
      var lunchChecked = $("input[name='lunch']:checked").length > 0 ? "yes" : "no";
      var dinnerChecked = $("input[name='dinner']:checked").length > 0 ? "yes" : "no";
